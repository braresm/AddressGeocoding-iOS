//
//  MapModel.swift
//  AddressGeocoding
//
//  Created by Balutoiu Rares Mihai on 21/03/2023.
//

import Foundation
import CoreLocation
import Combine
import MapKit

// Address Data Model
struct Address: Codable {
    let data: [Datum]
}

struct Datum: Codable {
    let latitude, longitude: Double
    let name: String?
}

// Our Pin Locations
struct Location: Identifiable {
    let id = UUID()
    let name: String
    let coordinate: CLLocationCoordinate2D
}

class MapModel: NSObject, ObservableObject, CLLocationManagerDelegate {
    private let BASE_URL = "http://api.positionstack.com/v1/forward"
    //private let API_KEY = "92d0989cebd26dea67f59db3a280d7a6"
    private let API_KEY = "89c080a8b24327547fc3baefec36e8f6"
    

    var locationManager: CLLocationManager?
    @Published var region = MKCoordinateRegion(center: CLLocationCoordinate2D(latitude: 44.4226, longitude: 26.1237), span: MKCoordinateSpan(latitudeDelta: 0.01, longitudeDelta: 0.01))
    @Published var isInit = false
    public var userRegion = MKCoordinateRegion(center: CLLocationCoordinate2D(latitude: 44.4226, longitude: 26.1237), span: MKCoordinateSpan(latitudeDelta: 0.01, longitudeDelta: 0.01))
    @Published var coordinates = []
    @Published var locations: [Location] = []
    @Published var suggestions: [AddressItem] = []
    public static let shared = MapModel(shared: true)
    
    init(shared: Bool = false) {
        super.init()
        
        if shared {
            self.checkIfLocationServiceIsEnables()
        }
    }
    
    // API request
    func getLocation(address: String, delta: Double, completionHandler: @escaping (_ addressItem: AddressItem) -> ()) {
        let pAddress = address.replacingOccurrences(of: " ", with: "%20")
        let url_string = "\(BASE_URL)?access_key=\(API_KEY)&query=\(pAddress)"
        
        guard let url = URL(string: url_string) else {
            print("Invalid URL")
            return }
        
        URLSession.shared.dataTask(with: url) { (data, response, error) in
            guard let data = data else {
                print(error!.localizedDescription)
                return }
            
            guard let newCoordinates = try? JSONDecoder().decode(Address.self, from: data) else { return }
            
            if newCoordinates.data.isEmpty {
                print("Could not find address...")
                return
            }
            
            // Set the new data
            DispatchQueue.main.async {
                self.suggestions = newCoordinates.data.filter({ suggestion in
                    return (suggestion.name != nil)
                })
                .map({ suggestion in
                    return AddressItem(alias: "\(String(describing: suggestion.name))", latitude: suggestion.latitude, longitude: suggestion.longitude)
                })
                
                let details = newCoordinates.data[0]
                let lat = details.latitude
                let lon = details.longitude
                                            
                self.coordinates = [lat, lon]
                self.region = MKCoordinateRegion(center: CLLocationCoordinate2D(latitude: lat, longitude: lon), span: MKCoordinateSpan(latitudeDelta: delta, longitudeDelta: delta))
//                self.region = MKCoordinateRegion(center: CLLocationCoordinate2D(latitude: lat, longitude: lon), latitudinalMeters: delta, longitudinalMeters: delta)
                
                let new_location = Location(name: "\(String(describing: details.name))", coordinate: CLLocationCoordinate2D(latitude: lat, longitude: lon))
                self.locations.removeAll()
                self.locations.insert(new_location, at: 0)
                
                completionHandler(AddressItem(alias: details.name ?? "No Address", latitude: details.latitude, longitude: details.longitude))
                
                print("Successfully loaded location! \(String(describing: details.name))")
            }
        }
        .resume()
    }
    
    func checkIfLocationServiceIsEnables() {
        if CLLocationManager.locationServicesEnabled() {
            locationManager = CLLocationManager()
            checkLocationAuthorization()
        } else {
            print("Location is not enabled")
        }
    }
    
    func checkLocationAuthorization() {
        guard let locationManager = locationManager else { return }
        
        switch locationManager.authorizationStatus {
        case .notDetermined:
            locationManager.requestWhenInUseAuthorization()
            locationManager.delegate = self
        case .restricted:
            print("Location is restricted")
        case .denied:
            print("Location is denied")
        case .authorizedAlways, .authorizedWhenInUse:
            region = MKCoordinateRegion(center: locationManager.location!.coordinate, span: MKCoordinateSpan(latitudeDelta: 0.01, longitudeDelta: 0.01))
            
            if !isInit {
                userRegion = MKCoordinateRegion(center: locationManager.location!.coordinate, span: MKCoordinateSpan(latitudeDelta: 0.01, longitudeDelta: 0.01))
                isInit = true
            }            
        @unknown default:
            break
        }
    }
    
    func locationManagerDidChangeAuthorization(_ manager: CLLocationManager) {
        checkLocationAuthorization()
    }
}
